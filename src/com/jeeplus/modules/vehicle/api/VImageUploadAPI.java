 ...
 */
@Controller
@RequestMapping(value = "v-api/upload")
public class VImageUploadAPI {
	...
	public String imageUpload( HttpServletRequest request,MultipartFile file) throws IllegalStateException, IOException {
        ...
        if (!file.isEmpty()) {                 
            	...
            	//创建文件夹
            	String realPath = saveImagePath + "/WXVImage/" + year + "/" + month + "/" + day + "/";	
            	FileUtils.createDirectory(realPath);
            	
                // 转存文件  
            	file.transferTo(new File(realPath + file.getOriginalFilename()));  
            	//文件保存路径  
            	picturePath = "/WXVImage/" + year + "/" + month + "/" + day + "/" + file.getOriginalFilename();
