...
<body class="gray-bg">
	...
	<div class="row">
	<div class="col-sm-12">
		<form:form id="searchForm" modelAttribute="vFans" action="${ctx}/vehicle/fans/list" method="post" class="form-inline">
	...
	<table id="contentTable" class="table table-striped table-bordered table-hover table-condensed dataTables-example dataTable">
		<thead>
			<tr>
				...
				<th  class="sort-column openId">微信OPENID</th>
		...
		<tbody>
		<c:forEach items="${page.list}" var="vFans">
			<tr>
				<td><img src='${vFans.avatar}' width="30px" height="30px"/></td>
				<td>${vFans.openId}</td>
				<td>${vFans.nickname}</td>
				<td>
					<c:if test='${vFans.sex eq "0"}'>未设置</c:if>
					<c:if test='${vFans.sex eq "1"}'>男</c:if>
					<c:if test='${vFans.sex eq "2"}'>女</c:if>
				</td>
				<td><fmt:formatDate value="${vFans.addtime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>
				<td><fmt:formatDate value="${vFans.updatetime}" pattern="yyyy-MM-dd HH:mm:ss"/></td>							
			</tr>
		</c:forEach>
		</tbody>
	</table>
